...
export class ModelServiceClient {
  ...
  initialize() {
    ...
    const modelServiceStubMethods =
        ['uploadModel', 'getModel', 'listModels', 'listModelVersions', 'updateModel', 'updateExplanationDataset', 'deleteModel', 'deleteModelVersion', 'mergeVersionAliases', 'exportModel', 'importModelEvaluation', 'getModelEvaluation', 'listModelEvaluations', 'getModelEvaluationSlice', 'listModelEvaluationSlices'];
 ...
 */
  async checkUpdateExplanationDatasetProgress(name: string): Promise<LROperation<protos.google.cloud.aiplatform.v1beta1.UpdateExplanationDatasetResponse, protos.google.cloud.aiplatform.v1beta1.UpdateExplanationDatasetOperationMetadata>>{
    const request = new operationsProtos.google.longrunning.GetOperationRequest({name});
    const [operation] = await this.operationsClient.getOperation(request);
    const decodeOperation = new gax.Operation(operation, this.descriptors.longrunning.updateExplanationDataset, gax.createDefaultBackoffSettings());

...
export class EndpointServiceClient {
  ...
  constructor(opts?: ClientOptions) {
    ...
    // pass `{fallback: true}` to the EndpointServiceClient constructor.
    this._gaxModule = opts.fallback ? gax.fallback : gax;

    // Create a `gaxGrpc` object, with any grpc-specific options
    // sent to the client.
    opts.scopes = (this.constructor as typeof EndpointServiceClient).scopes;
  ...
  /**
   * Check the status of the long running operation returned by the createEndpoint() method.
   * @param {String} name
  ...
  /**
   * Check the status of the long running operation returned by the deleteEndpoint() method.
   * @param {String} name
   *   The operation name that will be passed.
   * @returns {Promise} - The promise which resolves to an object.
   *   The decoded operation object has result and metadata field to get information from.
   ...
   *   The first element is Array of [Endpoint]{@link google.cloud.aiplatform.v1beta1.Endpoint} that corresponds to
   *   the one page received from the API server.
   *   If the second element is not null it contains the request object of type [ListEndpointsRequest]{@link google.cloud.aiplatform.v1beta1.ListEndpointsRequest}
   *   that can be used to obtain the next page of the results.
   *   If it is null, the next page does not exist.
   *   The third element contains the raw response received from the API server. Its type is
   *   [ListEndpointsResponse]{@link google.cloud.aiplatform.v1beta1.ListEndpointsResponse}.
   *
   *   The promise has a method named "cancel" which cancels the ongoing API call.
   */
  listEndpoints(

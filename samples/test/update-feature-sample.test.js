...
const entityTypeId = `entity_type_sample_${uuid()
  .replace(/-/g, '_')
  .slice(10, 20)}`;
const entityTypeDescription = 'created during update feature sample testing';
const featureId = `feature_sample_${uuid().replace(/-/g, '_').slice(10, 20)}`;
const featureDescription = 'created during update feature sample testing';
...
describe('AI platform update feature', async function () {
  this.retries(2);
  ...
  it('should update the created feature', async () => {

...
const entityTypeId = `entity_type_sample_${uuid()
  .replace(/-/g, '_')
  .slice(10, 20)}`;
const description = 'created during the create entity type sample testing';
...
describe('AI platform create entity type', async function () {
  this.retries(2);
  ...
  it('should create the entity type', async () => {
    const stdout = execSync(
      `node ./create-entity-type-sample.js ${project} ${featurestoreId} ${entityTypeId} "${description}" ${location} ${apiEndpoint}`
    );

...
const metadataSchemaUri = 'gs://google-cloud-aiplatform/schema/trainingjob/definition/custom_task_1.0.0.yaml';
const imageUri = 'gcr.io/cloud-aiplatform/prediction/tf2-cpu.2-1:latest';
const artifactUri = 'gs://prj-ucaip-tutorials-vcm/custom_model/hello-custom-sample/';
...
describe('AI platform upload model', () => {
  it('should upload the specified model', async () => {
    ...
    modelId = stdout.split(
        '/locations/us-central1/models/',
    )[1].split('\n')[0];
  });
  after('delete the model', async () => {
    execSync(
        `node ./delete-model.js ${modelId} ${project} ${location}`,
        {
          cwd,
        },
    );

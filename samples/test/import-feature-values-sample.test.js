...
describe('AI platform import feature values', async function () {
  this.retries(2);
  ...
  it('should import feature values', async () => {
    const stdout = execSync(
      `node ./import-feature-values-sample.js ${project} ${featurestoreId} ${entityTypeId} "${avroGcsUri}" ${entityIdField} ${featureTimeField} ${workerCount} ${location} ${apiEndpoint}`
    );

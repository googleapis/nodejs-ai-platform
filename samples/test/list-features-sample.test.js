...
const entityTypeId = `entity_type_sample_${uuid()
  .replace(/-/g, '_')
  .slice(10, 20)}`;
const entityTypeDescription = 'created during list features sample testing';
const featureId = `feature_sample_${uuid().replace(/-/g, '_').slice(10, 20)}`;
const featureDescription = 'created during list features sample testing';
...
describe('AI platform list features', async function () {
  this.retries(2);
